...
| Owner-scoped focus recovery (`recoverFocusFor`) — a teardown only recovers focus when *that* component was the one holding it, so a nested child's reconcile can't grab focus mid-construction of an unrelated component; a `{#if:destroy}` block's own teardown now also unregisters/recovers correctly when the destroyed content is a NESTED custom component, not just a plain element (`unregisterSubtree`, walked by live registry ownership rather than compile-time template visibility) | ✅ | GRAMMAR.md — "Focus system", `packages/compiler/runtime-assets/FocusManager` |
