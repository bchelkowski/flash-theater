sub Main()
    ...
    ' but the root Scene itself is created by THIS file (screen.CreateScene above), not by the
    ' router, so nothing else ever calls its own `public function setup()` for it. GRAMMAR.md's
    ' "Router" section is explicit about this: a router-mounted CHILD component (each chapter
    ' screen below) gets its own `setup()` called automatically the moment the router mounts it —
    ' but the root Scene's `setup()` (which is what actually calls `router.setRouting(...)` +
    ' `router.navigate(...)` in the first place) needs this one hand-written call, same
    ' one-hand-written-line convention as FlashTheaterSetupGlobals above. MUST run AFTER
    ' screen.show(), not before — confirmed live on a real device (Roku Ultra): scene.callFunc(...)
    ' issued on a Scene BEFORE screen.show() hangs indefinitely. See
